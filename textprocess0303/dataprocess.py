...
#id2label：['O', 'B-Test', 'I-Test', 'B-Disease', 'I-Disease'
#label2id：{'O': 0, 'B-Test': 1, 'I-Test': 2,
'''任务是：将文本的每个字都打上标记，将文本中的一个序列进行处理，要进行长短句处理
在此之前要进行处理，断句。用列表保存起来。
'''
def ischinese(char):
    if '\u4e00'<= char <='\u9fff':
        return True
...
def split_text(text):
    ...
    for i in range(len(split_index)-1):#对长句子进行拆分
        ...
        if e-b>150:
            for j in range(b,e):#现在走在的地方减去上次拆的地方超过了15字：
                if (j+1-other_index[-1])>15:#保证句子长度在15以上
                    if text[j]=='\n':
                        other_index.append(j+1)#换行后面拆
                    if text[j]==' ' and text[j-1].isnumeric() and text[j+1].isnumeric():
                        other_index.append(j+1)
    split_index += other_index #获得很多拆分的下标，又要重新进行排序
    split_index = list(sorted(set([0, len(text)] + split_index)))

    for i in range(1,len(split_index)-1):# 10   20  干掉全部是空格的句子
        idx=split_index[i] # idx=20， text[19]是空格
        while idx>split_index[i-1]-1 and text[idx-1].isspace():
            idx-=1
        split_index[i]=idx #将空格丢给下一个句子开头
    split_index = list(sorted(set([0, len(text)] + split_index)))


    #处理短句子
    temp_idx=[]
    i=0 #此时的i是需要自己去进行控制的，所以用while来处理
...
if __name__ == '__main__':
    ...
    # pattern='。|，|,|;|；|\.'
    # with open('ruijin_round1_train2_20181022/0.txt','r',encoding='utf8') as f:
    #     text=f.read()
    #     for m in re.finditer(pattern,text):
    #         # print(m)
    #         start=m.span()[0]-5
    #         end=m.span()[1]+5# span [0] 是前一个下标，span[1]是后一个下标
    ...
    # files=os.listdir(train_dir)
    # files=list(set([file.split('.')[0] for file in files]))
    # path = os.path.join(train_dir, files[2] + '.txt')
    # with open(path, 'r', encoding='utf8') as f:
    #     text = f.read()
    ...
    print(l) #m每个文章最后不一定都是\n结尾，可以去除，不会影响，只要后面没有数据即可。

'''1、本身数据打上标记   2、词语本身作为特征，词边距、标签、偏旁部首、拼音可以作为特征。'''
